...
//    - Select the first `li` element inside the `ul` with class `topics` and change the innerText to `all about document`
    let liFirstElement = document.body.children[5].children[0]
    liFirstElement.innerText = "all about document";
...
let inputCheckbox = document.querySelector('input[type="checkbox"][id="remember"]');
...
console.log(`The total number of button elements is ${allButtons.length}`);

// 18. Select all the `label` element and log the count.
let allLabels = document.querySelectorAll('label');
console.log(`The total number of label elements is ${allLabels.length}`);

// 19. Select all the elements with `id` of `test`
let allElementsWithIdTest = document.querySelectorAll('#test');

// 20. Select the first element with id `test` using `getElementById`
let firstElementWithIdTest = document.getElementById('test');
...
let parentElm = fieldsetElm.parentElement;
console.log(typeof parentElm);
